...
| 原生媒体 | 同源 C++20/libwebrtc；实际 UDP 对确认后才开放数据 | 引擎编译中，尚无真实远控会话 |
| Windows | 捕获、编码、解码、输入、隔离 worker、权限与单包升级 | 安全核心及输入适配已有构建测试；媒体集成待完成 |
...
| Android | 相同源 JNI、Surface、触控、IME、前后台与切网 | APK/JNI 构建及单元测试已通过；真机和实际解码待验证 |
